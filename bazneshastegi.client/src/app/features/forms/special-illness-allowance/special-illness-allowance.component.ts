...
import {SpecialIllnessAllowanceRequest} from './special-illness-allowance.model';

@Component({
  selector: 'app-special-illness-allowance',
  templateUrl: './special-illness-allowance.component.html',
  styleUrl: '../forms.scss',
  standalone: false
})
export class SpecialIllnessAllowanceComponent extends BaseFormComponent implements OnInit {
  columnsToDisplay = [
  ...
  submit() {
    ...
    if (this.form.valid && !this.relatedPersonIDError) {
      const request: SpecialIllnessAllowanceRequest = this.form.getRawValue();
      console.log('📌 فرم کمک هزینه بیماریهای خاص ثبت شد:', request);
